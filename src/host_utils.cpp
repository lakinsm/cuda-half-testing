...
HostUtils::HostUtils(const int &job_numbers) : _total(job_numbers)
{
	_current = 0;
}
...
void HostUtils::incrementProgressBar()
{
	...
	_current++;
	float progress = ((float)_current) / (float)_total;
	std::cout << '[';
	...
	std::cout << "] " << (int)(progress * 100.0f) << " % (" << _current << " / " << _total << ")\r";
	std::cout.flush();
}
